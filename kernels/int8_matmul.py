...
from kernels import require_triton
...
def _int8_matmul_fwd(
    ...
    for k0 in range(0, K, BLOCK_K):
        ...
        mask_k = k_idx < K
        x = tl.load(
            x_ptrs, mask=(offs_m[:, None] < M) & mask_k[None, :], other=0.0
        ).to(tl.float32)
        w_q = tl.load(
            w_ptrs, mask=(offs_n[:, None] < N) & mask_k[None, :], other=0
        ).to(tl.float32)
...
def int8_matmul(
    ...
    """Per-channel INT8 x FP16 matmul on CUDA."""
    if not x.is_cuda:
        raise RuntimeError("int8_matmul requires CUDA tensors")
    if scales.ndim != 1:
        raise ValueError("int8_matmul expects per-channel scales with shape (N,)")
