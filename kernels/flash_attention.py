...
def flash_attention(
    ...
    if D & (D - 1):
        raise ValueError(f"head_dim must be a power of 2 for the Triton kernel, got {D}")
    if D < 16:
        raise ValueError(f"head_dim must be >= 16 for the Triton kernel on sm_75, got {D}")
